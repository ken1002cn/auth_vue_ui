<template>
  <div class="login">
    ...
    <el-form ref="loginRef" :model="loginForm" :rules="loginRules" class="login-form">
      <h3 class="title">Django人事管理系统</h3>
...
<!-- 注册按钮 -->
    ...
    <div class="el-login-footer">
      <span>Copyright © 2024-2025 <a href="#" target="_blank">Gloomy</a> 版权所有.</span>
    </div>
  </div>
</template>
    ...
    loginRef.value.validate(async (valid)=>{
      if(valid){
        ...
        if(data.code==200){
          ...
          window.sessionStorage.setItem("currentUser",JSON.stringify(data.data.username)) //拿json里的字段
          window.sessionStorage.setItem("uid",JSON.stringify(data.data.id))
          router.replace("/")
        }else{
          ElMessage({
          message: '账号或密码错误',
